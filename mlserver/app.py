    ...
    for filename in os.listdir(MODELS_DIR):
        if filename.endswith(".joblib"):
            ...
            try:
                ...
                app.logger.info(
                    f"Successfully loaded model '{model_name}' which expects {len(input_features)} input features."
                )
...
def predict():
    ...
    arches = [x["kubernetes.io/arch"] for x in feature_matrix]
    ...
    if metric_name not in models:
        return (
            jsonify(
                {
                    "error": f"Model '{metric_name}' not found. Available models: {list(models.keys())}"
                }
            ),
            404,
        )
    ...
    feature_matrix = [
        x for x in feature_matrix if "node-role.kubernetes.io/control-plane" not in x
    ]
    instances = [
        (
            x["node.kubernetes.io/instance-type"]
            if "node.kubernetes.io/instance-type" in x
            else i
        )
        for i, x in enumerate(feature_matrix)
    ]
    feature_matrix = [
        {k: v for k, v in x.items() if k in model_info["features"]}
    ...
    # We will likely always have extra labels (features)
